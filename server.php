...
// Если adi das — видит переписку с любым
  ...
  function setToDB($data){
    ...
    $name = $this->escapeStr($data['name'], 33);
    $text = $this->escapeStr($data['message_text'], 1111);

    if ($data['destination']){
      $destination = $this->escapeStr($data['destination'], 33);
      echo ($destination);
    } else {
      $destination = 'adi das';
    }
...
class Chat implements MessageComponentInterface {
  ...
  public function onOpen(ConnectionInterface $conn) {
    $this->clients->attach($conn); // ⬅️ ОБЯЗАТЕЛЬНО!
    echo "Новое соединение: {$conn->resourceId}\n";
  }


  public function onMessage(ConnectionInterface $from, $msg) {
   // echo "Сырое сообщение от клиента: $msg\n";

    $data = json_decode($msg, true);

    // Проверка на корректность структуры
    if (!is_array($data) || empty($data['name']) || empty($data['message_text'])) {
    ...
    if (!$this->chatHandler->setToDB($data))return;

    // Рассылка всем клиентам (включая отправителя)
